...
这是一个音频录制的实验，在 STM32N647 开发板上实现了录音机的功能。

本实验对应的工程文件夹为：`<STM32N647 开发板软件包路径>/Projects/45_Recorder`。
...
按下 `KEY0` 按键，可开始录音，音频将通过 `MIC1` 进行采集。

按下 `KEY1` 按键，可停止录音，录制的音频文件将保存在 TF 卡中。